pmod <- function(formula, data, prefix) {
...
getPars <- function(object, ...) {
	object$pars
}

setPars <- function(object,values, ...) {
...
predpmod <- function(object, pars) { return(object$x %*% pars) }

predict.pmod <- function(object, ...) { return(object$x %*% object$pars) }

reorderDrift <- function(resp, drift) {
	nresp <- ncol(drift)
	nn <- nrow(drift)
...
obj <- function(rt, pars, loglink, weights) {
    ...
    , x0max = pars[, 2]
    , chi = pars[, 2] + pars[, 3]
    , sdI = pars[, 1] # sdI=0.15, # Scott: I fit chi-x0max.
