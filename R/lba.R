lba <- function(
  ...
  , debug = FALSE
	...
	# create submodels 
	smod <- pmod(s, data = data, prefix = "s")
	Amod <- pmod(A, data = data, prefix = "A")
	bmod <- pmod(b, data = data, prefix = "b")
	t0mod <- pmod(t0, data = data, prefix = "t0")
  
	multiDrift <- FALSE
	if(is.list(vs)) {
		multiDrift <- TRUE
		nvsModels <- length(vs)
		if(nvsModels != ncat) stop("Nr of 'vs' models should equal the number of categories in the 'response' variable")
		vsModels <- list()
		for(i in 1:ncat) {
			vsModels[[i]] <- pmod(vs[[i]], data = data, prefix = paste("vs", i, sep = ""))
		}
	} else {
		drmod <- pmod(vs, data = data, prefix = "vs")
	}
	
	if(multiDrift) {
		models <- list(smod, Amod, bmod, t0mod)
		for(i in 1:ncat) {
			models[[4 + i]] <- vsModels[[i]]
		}
	} else {
		models <- list(smod, Amod, bmod, t0mod, drmod)
	}
...
# 		allpars[2] <- 0.5*allpars[3]
		...
		parsMat[, 5:(4 + ncat)] <- reorderDrift(resp, parsMat[, 5:(4 + nvs)])
...
# 			lower <- rep(0, length(pars))
      ...
      , control = list(maxit = maxit, trace = 1, fnscale = -1)
		...
		# set A to it's appropriate value
		# res$par[2] <- 0.5*res$par[3]
		
		if(res$convergence == 0) {
			if(hessian) {
				res$hessian <- -res$hessian # needed because of maximization done instead of minimization
